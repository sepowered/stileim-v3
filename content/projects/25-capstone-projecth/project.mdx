...
- 환경 분석: 3C, SWOT, 포지셔닝, 카드 소팅, 경쟁사 분석
- 리서치: 제주 현장 조사, 사용자 여정 분석
- 설계: F/O 스토리보드, 온보딩·매칭 플로우, IA 구조
- 전략: 콘텐츠 기반 Retention 구조 설계
- 기술 기획: LLM + RAG 기반 라이프스타일 매칭 모델
- 비즈니스 기획: 로컬 포인트, 자원봉사 예우 구조, 지역경제 순환 모델
